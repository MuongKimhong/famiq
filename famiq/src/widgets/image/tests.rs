...
fn test_create_default_image() {
    ...
    let img_q = app.world_mut().query::<(&WidgetId, &IsFamiqImage)>().single(app.world());
...
fn test_create_image_with_class() {
    ...
    let img_q = app.world_mut().query::<(&WidgetClasses, &IsFamiqImage)>().single(app.world());
    assert!(img_q.is_ok(), "There should be only 1 Image");
...
fn test_create_image_with_custom_size() {
    ...
    let img_q = app.world_mut().query::<(&Node, &IsFamiqImage)>().single(app.world());
